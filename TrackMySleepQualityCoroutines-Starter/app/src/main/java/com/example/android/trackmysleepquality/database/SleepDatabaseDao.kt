...
interface SleepDatabaseDao {
     ...
     * doesn't already exist, this method will not change the database. Called by the `update` method
     * of `SleepTrackerViewModel`, which is called by its `onStopTracking` method, which is called
     * by a binding expression for the "android:onClick" attribute of the `Button` with ID R.id.stop_button
     * in the file layout/fragment_sleep_tracker.xml which is the layout file for `SleepTrackerFragment`.
     *
