...
class MainActivity : AppCompatActivity() {
     ...
     * `windowInsets` with [WindowInsetsCompat.Type.systemBars] as the argument. It
     * then gets the insets for the IME (keyboard) using [WindowInsetsCompat.Type.ime].
     * It then updates the layout parameters of `v` to be a [ViewGroup.MarginLayoutParams]
     * with the left margin set to `systemBars.left`, the right margin set to `systemBars.right`,
     * the top margin set to `systemBars.top`, and the bottom margin set to the maximum of the
