 ...
 * the layout files layout/fragment_sleep_detail.xml and layout/list_item_sleep_night_linear.xml
 * to set the text of the two [TextView]'s.
 *
...
/**
 * Returns a string describing the quality of sleep based on the numeric quality rating.
 * Used by the `TextView.setSleepQualityString` extension function, which is used as the
 * `BindingAdapter` for the "app:sleepQualityString" attribute which is used on the three
 * [TextView]'s with ID R.id.quality_string in the layout files layout/fragment_sleep_detail.xml,
 * layout/list_item_sleep_night.xml, and layout/list_item_sleep_night_linear.xml to set the
 * text of the [TextView]. We use a `when` switch to set our [String] variable `var qualityString`
 * to different strings from our resources, then return `qualityString` to the caller.
 *
 ...
 * nicely formatted string for display. Used by our [formatNights] method, which used to be used
 * to set the `nightsString` field of `SleepTrackerViewModel` before the app was converted to use
 * `RecyclerView`.
 *
 ...
 * device is older.
 *
 * Used to be used to set the `nightsString` field of `SleepTrackerViewModel` before the app was
 * converted to use `RecyclerView`.
 *
 ...
 *
 * Used to be used for the initial steps in the code lab.
 *
 * @param textView the [TextView] to use as our view.
 */
@Suppress("unused", "MemberVisibilityCanBePrivate") // Unused but instructional
