...
class SleepTrackerViewModel(
     ...
     * the caller. Called by our [initializeTonight] method and by our [onStartTracking] method when
     * the user clicks the "Start" button (a binding expression for the "android:onClick" attribute
     * of the button calls [onStartTracking]).
     *
     ...
     * added to our [navigateToSleepQuality] field in the `onCreateView` override of
     * `SleepTrackerFragment` after if navigates to `SleepQualityFragment`.
     */
    fun doneNavigating() {
     ...
     * added to it in the `onCreateView` override of `SleepTrackerFragment`.
     ...
     * binding expression for the "android:onClick" attribute of the "STOP" button in the layout
     * file layout/fragment_sleep_tracker.xml when the user clicks that button.
     *
     ...
     * to `true` causing an `Observer` of [showSnackBarEvent] added to it in the `onCreateView` override
     * of `SleepTrackerFragment` to post a `SnackBar` informing the user that his data is gone.
     * Executes when the CLEAR button is clicked because of a binding expression for the
     * "android:onClick" attribute of the button R.id.clear_button in the layout file
     * layout/fragment_sleep_tracker.xml
     */
    fun onClear() {
        uiScope.launch {
     ...
     * added to our [showSnackBarEvent] field in the `onCreateView` override of `SleepTrackerFragment`
     * after it shows its Snackbar.
     */
